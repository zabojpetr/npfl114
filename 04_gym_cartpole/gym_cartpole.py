...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            # Computation
            observation = tf.layers.flatten(self.observations, name="observation")
            hl1 = tf.layers.dense(observation, 4, activation=tf.nn.relu, name="hidden_layer1")
            hidden_layer = tf.layers.dense(hl1, 2, activation=tf.nn.relu, name="hidden_layer2")
            ...
            self.training = tf.train.AdamOptimizer(0.0001).minimize(loss, global_step=global_step, name="training")
...
if __name__ == "__main__":
    ...
    for i in range(args.epochs):
        ...
        for j in range(25):
            ...
            network.train(observations[perm], labels[perm])
