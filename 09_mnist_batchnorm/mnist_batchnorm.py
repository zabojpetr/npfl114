    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            for layer in args.cnn.split(','):
                items = layer.split('-')
                if items[0] == 'CB':
                    layers.append(tf.layers.conv2d(layers[-1], int(items[1]), (int(items[2]), int(items[2])), (int(items[3]), int(items[3])), items[4], use_bias=False, name="Layer" + str(len(layers))))
                    layers.append(tf.nn.relu(tf.layers.batch_normalization(layers[-1], training=self.is_training, name="Layer" + str(len(layers)))))
...
if __name__ == "__main__":
    ...
    # test --cnn arg
    # args.cnn = "CB-10-3-2-same,M-3-2,F,R-100"
