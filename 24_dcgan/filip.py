#!/usr/bin/env python3
# 1645400d-2116-11e8-9de3-00505601122b
import numpy as np
...
class Network:
    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def generator(z):
                ...
                hidden_layer1 = tf.layers.dense(z, 1024, activation=None, use_bias=False)
                hidden_layer1_norm = tf.layers.batch_normalization(hidden_layer1, training=True)
                ...
                hidden_layer2 = tf.layers.dense(hidden_layer1_norm_relu, 7 * 7 * 64, activation=None, use_bias=False)
                hidden_layer2_norm = tf.layers.batch_normalization(hidden_layer2, training=True)
                ...
                cl1 = tf.layers.conv2d_transpose(imgs, filters=32, kernel_size=(5, 5), strides=2, padding="same",
                ...
                cl2 = tf.layers.conv2d_transpose(cl1_norm_relu, filters=1, kernel_size=(5, 5), strides=2,
            ...
            def discriminator(image):
                ...
                cl1 = tf.layers.conv2d(image, filters=32, kernel_size=(5, 5), padding="same",
                                       activation=None, use_bias=False)
                cl1_norm = tf.layers.batch_normalization(cl1, training=True)
                cl1_norm_relu = tf.nn.relu(cl1_norm)

                # - max pooling layer with kernel size 2 and stride 2
                mp1 = tf.layers.max_pooling2d(cl1_norm_relu, pool_size=2, strides=2)

                # - batch normalized conv2d with 64 output channels, kernel size 5,
                #   "same" padding and ReLU activation
                cl2 = tf.layers.conv2d(mp1, filters=64, kernel_size=(5, 5), padding="same",
                                       activation=None, use_bias=False)
                cl2_norm = tf.layers.batch_normalization(cl2, training=True)
                cl2_norm_relu = tf.nn.relu(cl2_norm)

                # - max pooling layer with kernel size 2 and stride 2
                mp2 = tf.layers.max_pooling2d(cl2_norm_relu, pool_size=2, strides=2)

                # - flattening layer
                flattened_image = tf.layers.flatten(mp2, name="flatten")

                # - batch normalized dense layer with 1024 neurons and ReLU activation
                hidden_layer1 = tf.layers.dense(flattened_image, 1024, activation=None, use_bias=False)
                hidden_layer1_norm = tf.layers.batch_normalization(hidden_layer1, training=True)
                hidden_layer1_norm_relu = tf.nn.relu(hidden_layer1_norm)

                # - (non-normalized) dense layer with 1 neuron without activation.
                hidden_layer2 = tf.layers.dense(hidden_layer1_norm_relu, 1, activation=None)
                #
            ...
            self.generator_loss = tf.losses.sigmoid_cross_entropy(tf.ones(tf.shape(discriminator_logit_fake)),
                                                                  discriminator_logit_fake)
            ...
            with summary_writer.as_default(), tf.contrib.summary.record_summaries_every_n_global_steps(100):
                self.discriminator_summary = [
                    tf.contrib.summary.scalar("gan/discriminator_loss", self.discriminator_loss),
            ...
            with summary_writer.as_default(), tf.contrib.summary.always_record_summaries():
                self.generated_image_summary = tf.contrib.summary.image("gan/generated_image",
                                                                        tf.expand_dims(self.generated_image_data,
                                                                                       axis=0))
    ...
    def train(self, images):
        ...
        discriminator_loss = \
            self.session.run([self.discriminator_training, self.discriminator_summary, self.discriminator_loss],
                             {self.images: images, self.z: self.sample_z(images.shape[0])})[-1]
        ...
        generator_loss = \
            self.session.run([self.generator_training, self.generator_summary, self.generator_loss],
                             {self.z: self.sample_z(len(images))})[-1]
...
if __name__ == "__main__":
    ...
    if args.recodex:
        data = mnist.input_data.read_data_sets(".", reshape=False, validation_size=args.recodex_validation_size,
                                               seed=42)
