...
class Network:
    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def generator(z):
                ...
                d1 = tf.layers.dense(z, 1024, use_bias=False)
                bn1 = tf.nn.relu(tf.layers.batch_normalization(d1, training=True))
                d2 = tf.layers.dense(bn1, 7*7*64,  use_bias=False)
                bn2 = tf.nn.relu(tf.layers.batch_normalization(d2, training=True))
                conv1 = tf.layers.conv2d_transpose(tf.reshape(bn2, [-1,7,7,64]), filters=32, kernel_size=5, strides=2, padding="same", use_bias=False)
                bn3 = tf.nn.relu(tf.layers.batch_normalization(conv1, training=True))
                conv2 = tf.layers.conv2d_transpose(bn3, filters=1, kernel_size=5, strides=2, padding="same", activation=tf.nn.sigmoid)
                return conv2
            ...
            def discriminator(image):
                ...
                conv1 = tf.layers.conv2d(image, filters=32, kernel_size=5, padding="same", use_bias=False)
                bn1 = tf.nn.relu(tf.layers.batch_normalization(conv1, training=True))
                mp1 = tf.layers.max_pooling2d(bn1, pool_size=2, strides=2)

                conv2 = tf.layers.conv2d(mp1, filters=64, kernel_size=5, padding="same", use_bias=False)
                bn2 = tf.nn.relu(tf.layers.batch_normalization(conv2, training=True))
                mp2 = tf.layers.max_pooling2d(bn2, pool_size=2, strides=2)

                flatten = tf.layers.flatten(mp2)
                d1 = tf.layers.dense(flatten, 1024, use_bias=False)
                bn3 = tf.nn.relu(tf.layers.batch_normalization(d1, training=True))
                d2 = tf.layers.dense(bn3, 1)
            ...
            with tf.variable_scope("discriminator"):
                ...
                # self.images = tf.Print(self.images,[tf.shape(self.images)], "images: ")
            ...
            with tf.variable_scope("discriminator", reuse = True):
                ...
                # self.generated_images = tf.Print(self.generated_images,[tf.shape(self.generated_images), "generated_images: "])
    ...
    def train(self, images):
        ...
        _, _, discriminator_loss = self.session.run(
