    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            if args.rnn_cell == "LSTM":
                rnn_cell = tf.nn.rnn_cell.BasicLSTMCell(args.rnn_cell_dim, name="LSTM")
            elif args.rnn_cell == "GRU":
                rnn_cell = tf.nn.rnn_cell.GRUCell(args.rnn_cell_dim, name="GRU")
            else:
                rnn_cell = tf.nn.rnn_cell.BasicRNNCell(args.rnn_cell_dim, name="RNN")
            ...
            for i in range(self.TRAIN):
                ...
                hidden_layer, state = rnn_cell(tf.reshape(self.sequence[i-1],[1,1]) if i-1 >= 0 else tf.zeros([1,1], tf.float32), state)
                predictions.append(tf.reshape(dense(hidden_layer),[]))
            ...
            for i in range(self.TEST):
                ...
                hidden_layer, state = rnn_cell(tf.reshape(predictions[-1],[1,1]), state)
                predictions.append(tf.reshape(dense(hidden_layer),[]))
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        _, loss = network.prediction_summary(data, network.predict(data[:Network.TRAIN]))
