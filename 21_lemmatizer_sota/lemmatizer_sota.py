...
class Network:
    ...
    def construct(self, args, source_chars, target_chars, bow, eow):
        with self.session.graph.as_default():
            ...
            source_encoded = tf.layers.batch_normalization(source_encoded, training=self.isTraining)
            source_states = tf.layers.batch_normalization(source_states, training=self.isTraining)
...
if __name__ == "__main__":
    ...
    # Load the data
    train = morpho_dataset.MorphoDataset("../18_tagger_sota/czech-pdt-train.txt", max_sentences=1000)
