#!/usr/bin/env python3
# Petr Záboj - 2784945b-2116-11e8-9de3-00505601122b
# Filip Rafaj - 1645400d-2116-11e8-9de3-00505601122b
import numpy as np
...
class Network:
    ...
    def construct(self, args, source_chars, target_chars, bow, eow):
        with self.session.graph.as_default():
            ...
            source_encoded, source_states = tf.nn.bidirectional_dynamic_rnn(rnn_cell_fwd, rnn_cell_bwd, source_embed, self.source_seq_lens, dtype=tf.float32)
            ...
            class DecoderTraining(tf.contrib.seq2seq.Decoder):
                ...
                def initialize(self, name=None):
                    finished = tf.map_fn(lambda x: tf.cond(x > 0, lambda: False, lambda: True), target_lens, tf.bool) # TODO: False if target_lens > 0, True otherwise
                    states = source_states # TODO: Initial decoder state to use
                    inputs = with_attention(tf.nn.embedding_lookup(target_embedings, tf.fill([self.batch_size], bow)), states) # TODO: Call with_attention on the embedded BOW characters of shape [self.batch_size].
                             # You can use tf.fill to generate BOWs of appropriate size.
                    return finished, inputs, states

                def step(self, time, inputs, states, name=None):
                    outputs, states = rnn_decoder(inputs, states) # TODO: Run the decoder GRU cell using inputs and states.
                    outputs = decoder_layer(outputs) # TODO: Apply the decoder_layer on outputs.
                    next_input = with_attention(target_embed[:,time], states) # TODO: Next input is with_attention called on words with index `time` in target_embedded.
                    finished = tf.map_fn(lambda x: tf.cond(x > time + 1, lambda: False, lambda: True), target_lens, tf.bool) # TODO: False if target_lens > time + 1, True otherwise.
            ...
            class DecoderPrediction(tf.contrib.seq2seq.Decoder):
                ...
                def step(self, time, inputs, states, name=None):
                    outputs, states = rnn_decoder(inputs, states) # TODO: Run the decoder GRU cell using inputs and states.
                    outputs = decoder_layer(outputs)# TODO: Apply the decoder_layer on outputs.
                    outputs = tf.cast(tf.argmax(outputs, -1),tf.int32) # TODO: Use tf.argmax to choose most probable class (supply parameter `output_type=tf.int32`).
                    next_input = with_attention(tf.nn.embedding_lookup(target_embedings, outputs), states) # TODO: Embed `outputs` using target_embeddings and pass it to with_attention.
                    finished = tf.map_fn(lambda x: tf.cond(tf.equal(x, eow), lambda: True, lambda: False), outputs, tf.bool) # TODO: True where outputs==eow, False otherwise
...
if __name__ == "__main__":
    ...
    # Load the data
    if not args.recodex:
        train = morpho_dataset.MorphoDataset("../19_lemmatizer_noattn/czech-cac-train.txt", max_sentences=5000)
        dev = morpho_dataset.MorphoDataset("../19_lemmatizer_noattn/czech-cac-dev.txt", train=train, shuffle_batches=False)
    else:
