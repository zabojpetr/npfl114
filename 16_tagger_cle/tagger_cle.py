...
class Network:
    ...
    def construct(self, args, num_words, num_chars, num_tags):
        with self.session.graph.as_default():
            ...
            w_embeddings = tf.get_variable("embeddings", [num_words, args.we_dim])
            ...
            ch_fwd = tf.nn.rnn_cell.GRUCell(args.cle_dim, name="GRU_ch_fwd")
            ch_bwd = tf.nn.rnn_cell.GRUCell(args.cle_dim, name="GRU_ch_bwd")
            _, ch_state = tf.nn.bidirectional_dynamic_rnn(ch_fwd, ch_bwd, ch_embed, self.charseq_lens, dtype=tf.float32)

            # TODO: Sum the resulting fwd and bwd state to generate character-level word embedding (CLE)
            # of unique words in the batch.
            ch_sum = tf.add(*ch_state)
            ...
            cles = tf.nn.embedding_lookup(ch_sum, self.charseq_ids)

            # TODO: Concatenate the word embeddings (computed above) and the CLE (in this order).
            concat_embedings = tf.concat([w_embed, cles], 2)
            ...
            outputs, _ = tf.nn.bidirectional_dynamic_rnn(fwd, bwd, concat_embedings, self.sentence_lens, dtype=tf.float32)
