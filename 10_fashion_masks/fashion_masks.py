...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            self.my_masks = tf.placeholder(tf.float32, [None, self.HEIGHT, self.WIDTH, self.LABELS])
            ...
            output_layer = tf.layers.dense(layers[-1], self.LABELS * 28*28, activation=tf.nn.relu, name='output_layer')
            rs = tf.reshape(output_layer,(-1,self.HEIGHT, self.WIDTH, self.LABELS))
            rsum = tf.reduce_sum(rs,axis=(1,2))

            self.labels_predictions = tf.argmax(rsum, axis=1)
            self.mp = tf.reshape(tf.cast(tf.cast(output_layer, tf.bool), tf.float32), (-1,28,28,10))
            self.masks_predictions = tf.expand_dims(tf.map_fn(lambda x: x[:,:,tf.argmax(tf.reduce_sum(x,axis=(0,1)))], self.mp),-1)

            loss = tf.losses.sigmoid_cross_entropy(tf.reshape(self.my_masks,(-1,7840)), output_layer)
...
if __name__ == "__main__":
    ...
    with open("fashion_masks_test_{}_setting.txt".format(date), "w") as setting_file:
    ...
    with open("fashion_masks_test_{}.txt".format(date), "w") as test_file:
