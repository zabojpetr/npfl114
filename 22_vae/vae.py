    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def encoder(image):
                ...
                d1 = tf.layers.dense(flatten, 500, tf.nn.relu)
                d2 = tf.layers.dense(d1, 500, tf.nn.relu)

                z_mean = tf.layers.dense(d2,self.z_dim)
                z_log_variance = tf.layers.dense(d2,self.z_dim)
            ...
            def decoder(z):
                ...
                d1 = tf.layers.dense(z, 500, tf.nn.relu)
                d2 = tf.layers.dense(d1, 500, tf.nn.relu)
                pixels = tf.layers.dense(d2, self.HEIGHT * self.WIDTH)

                return tf.reshape(pixels, [-1, self.WIDTH, self.HEIGHT, 1])
            ...
            kl = tf.distributions.kl_divergence(tf.distributions.Normal(z_mean, z_sd), tf.distributions.Normal(0.,1.))
            # kl = tf.Print(kl, [kl])
            latent_loss = tf.reduce_mean(kl, axis=[0,1])
            ...
            self.loss = self.WIDTH * self.HEIGHT * reconstruction_loss + latent_loss * self.z_dim
