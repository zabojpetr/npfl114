...
class Network:
    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def generator(z):
                ...
                d1 = tf.layers.dense(z, 128, tf.nn.relu)
                d2 = tf.layers.dense(d1, self.WIDTH * self.HEIGHT, tf.nn.sigmoid)

                return tf.reshape(d2, [-1,self.WIDTH, self.HEIGHT, 1])
            ...
            def discriminator(image):
                ...
                flatten = tf.layers.flatten(image, name="flatten")
                d1 = tf.layers.dense(flatten, 128, tf.nn.relu)
    ...
    def train(self, images):
        ...
        _,_,discriminator_loss = self.session.run([self.discriminator_training, self.discriminator_summary,  self.discriminator_loss], {self.images: images, self.z: self.sample_z(images.shape[0])})
        ...
        _,_,generator_training = self.session.run([self.generator_training, self.generator_summary, self.generator_loss],{self.z: self.sample_z(images.shape[0])})
