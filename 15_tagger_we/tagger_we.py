...
class Network:
    ...
    def construct(self, args, num_words, num_tags):
        with self.session.graph.as_default():
            ...
            embeddings = tf.get_variable("embeddings", [num_words, args.we_dim])
            # print("embedings", embeddings.shape)

            # TODO: Embed self.word_ids according to the word embeddings, by utilizing
            # `tf.nn.embedding_lookup`.
            embed = tf.nn.embedding_lookup(embeddings, self.word_ids)
            ...
            outputs, _ = tf.nn.bidirectional_dynamic_rnn(fwd, bwd, embed, self.sentence_lens, dtype=tf.float32)
